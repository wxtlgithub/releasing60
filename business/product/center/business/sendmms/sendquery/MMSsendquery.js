    ...
    if (Ext.get("Js.Center.SendMMS.MMSSendQuery.MainPanel") == null) {
        ...
        var cm = new Ext.grid.ColumnModel([{
            ...
            renderer: function(value, meta, record, rowIndex, colIndex, store) {
                var row = Js.Center.SendMMS.MMSSendQuery.DisplayStore.getAt(rowIndex);
                if (row.get("vc2status") == 2 || row.get("vc2status") == 99 || row.get("vc2status") == -99 || row.get("vc2status") == -1|| row.get("vc2status") == 3) {
                    return "审核完成";
                }
                else {
                    if (value == 0) {
            ...
            renderer: function(value, meta, record, rowIndex, colIndex, store) {
                ...
                if (row.get("numsendtype") == 5 || row.get("numsendtype") == 4 || row.get("numsendtype") == 3) {
                    if(row.get("numsuccess")==0){
                        ...
                        strsuccess="<a href='#' onclick='exportData(\"" + Js.Center.SendMMS.MMScheckQueryURL + "\",\"id=" + value + "&flag=selectexport&successtype=1\")'><font color='green'>" + row.get("numsuccess") + "</font></a>/";
                    }
                    if(row.get("numfailed") == 0){
                    	strfailed="<font color='red'>0</font>";
                    } else {
                    strfailed ="<a href='#' onclick='exportData(\"" + Js.Center.SendMMS.MMScheckQueryURL + "\",\"id=" + value + "&flag=selectexport&successtype=0\")'><font color='red'>" + row.get("numfailed") + "</font></a>";
                    
                    }
                    return strsuccess+strfailed;
                    //return "<a href='#' onclick='exportData(\"" + Js.Center.SendMMS.MMScheckQueryURL + "\",\"id=" + value + "&flag=selectexport&successtype=1\")'><font color='green'>" + row.get("numsuccess") + "</font></a>/<a href='#' onclick='exportData(\"" + Js.Center.SendMMS.MMScheckQueryURL + "\",\"id=" + value + "&flag=selectexport&successtype=0\")'><font color='red'>" + row.get("numfailed") + "</font></a>";
                
                }else if(row.get("numsendtype") == 2){
                	return "<font color='green'>" + row.get("numtotal") + "</font>/<font color='red'>0</font>";
                } else {
                    return "<font color='green'>" + row.get("numsuccess") + "</font>/<font color='red'>0</font>";
                }
            }
        }, {
            header: "发送状态",
                ...
                items: [{
                    ...
                    items: [{
                        ...
                        items: [{
                            ...
                            validator: function() {
                                var strat_time = Ext.get("sendquerydattart").dom.value;
                   ...
                   store: new Ext.data.SimpleStore({
                       fields: ["show", "value"],
                       data: [["-=请选择=-", ""], ["待审核", "num_0"], ["一审通过", "num_1"], ["二审通过", "num_2"], ["审核驳回", "num_3"], ["彩信过期", "vc2_-1"], ["未发送", "vc2_1"], ["已发送", "vc2_3"], ["待发送", "vc2_99"]]
                   })
}]
}]
}]
                        ...
                        Js.Center.SendMMS.MMSSendQuery.queryGrid = function() {
                            if (selectPanel.getForm().isValid()) {
                                ...
                                var _secondcheckid = Ext.get("sendquerysecondcheckid").getValue();
